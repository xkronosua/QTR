...
class DesignerMainWindow(QtGui.QMainWindow, Ui_MplMainWindow):
    def __init__(self, parent = None):
        ...
        QtCore.QObject.connect(self.mplactionOpen,
                               QtCore.SIGNAL("triggered()"),
                               self.select_file)
        QtCore.QObject.connect(self.mplactionSave,
                               QtCore.SIGNAL("triggered()"),
                               self.save_file)
    ...
    def update_graph(self):
        ...
        if tmp1 != self.dlockx or tmp2 != self.dlocky:
            self.background = None
        # save current plot variables
        self.dlockx = self.mplspinBox.value()-1
        self.dlocky = self.mplspinBox_2.value()-1
    ...
    def on_press(self, event):
        ...
        if self.issecond == 1 :
            ...
            if self.y3 >= y:
                ...
                for i in range(len(self.tdata[:, self.mplspinBox.value()-1])):
                    x = self.tdata[i, self.mplspinBox.value()-1]
                    y = ((self.y2 - self.y1) / (self.x2 - self.x1)) * \
                        (x - self.x2) + self.y2
                    if self.tdata[i, self.mplspinBox_2.value()-1] >= y \
        ...
        if self.issecond == 1 :
            ...
            if self.y3 <= self.y1 and self.y3 >= self.y2 \
               and self.x3 >= self.x1 and self.x3 <= self.x2 :
                ...
                for i in range(len(self.tdata[:,self.mplspinBox.value()-1])):
                    x = self.tdata[i, self.mplspinBox.value()-1]
                    y = self.tdata[i, self.mplspinBox_2.value()-1]
