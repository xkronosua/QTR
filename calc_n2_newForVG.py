...
def calcReChi3(XY, m=4, a=24,Lambda=532, n0=1.5, d=50*10**(-4), z=68, L=14, f=11, r0=0.1):
	...
	*)
	...
	X, Y = XY.T
	A, B1, B2, B3, B4, *Bn = np.polyfit(X, Y, m)[::-1]
	print(A,B1,B2,B3,B4, Bn)
	#In[8]= 
	...
	#In[18]= ww[x_, a_, F_, ld_] = a*cSqrt((1 - x/F)**2 + (x/ld)**2) 
	...
	#f = 11
	r02 = r0**2
	ww = lambda x, a, F, ld : a*cSqrt((1 - x/F)**2 + (x/ld)**2)
	Rc = lambda x, rad, ld: rad*((1 - x/rad)**2 + (x/ld)**2)/(1 - x/rad*(1 + (rad/ld)**2))
	...
	Th3 = 1/(Norm1*8)*(1/3*exp(-8*r02*(7 + b2)/(ar2*(49 + b2)))*
	    ...
	    exp(-8*r02*(15 + b2)*(1 + b2)/(ar2*(25 + b2)*(9 + b2)))*
	     sin(16*r02*b*(1 + b2)/(ar2*(25 + b2)*(9 + b2))) )*(1/cSqrt(4))
	...
	Th4 = ((1/4.)*(1 - exp(-10*r02*(1 + b2)/(25 + b2))) -
	     ...
	     (1/12.)*(1 - exp(-10*r02*(9 + b2)/(81 + b2)))*
	      cos(80*r02*b/(81 + b2)))/(Norm1*5)*(1/cSqrt(5))
	...
	n22 = cSqrt((B2/A)*(Th0/Th2))/c1*10**(-3)
