...
class QTR(QtGui.QMainWindow):
	...
	def Undo(self):
		...
		senderName = self.sender().objectName()
		if len(self.dataStack[Dict[str(senderName)][0]])>=2:
			Type = Dict[str(senderName)][0]
			self.updateData(Type = Type, action = -1)
			
		else:
			Dict[str(senderName)][1].setEnabled(False)
	...
	def Reset(self):
		...
		else: Dict[str(senderName)][1].setEnabled(False)
	...
	def pathTextChanged(self, text):
		...
		sender = self.sender()
		active = Dict[str(sender.objectName())]
		...
		if os.path.exists(path):
			try:
				...
				if activeFilt[0].isEnabled() and activeFilt[1].isEnabled():
					...
					for i in (0,1):
						filtNames = str(activeFilt[i].text()).strip().replace(" ","").upper()
						temp = 1.
