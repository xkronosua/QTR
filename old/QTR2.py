...
class QTR(QtGui.QMainWindow):
	...
	Path = ['','','']
	Root = os.getcwd()
	FiltersPath = os.path.join(Root, "./filters.csv")
	...
	########################  Обробка  #################################
	####################################################################
	def poly_cut(self, data, p, d=0.7, m=6, precision = 0.75):
	...
	def polyCut(self):
		...
		data = self.poly_cut(XY, active[1], active[2], active[3])
		self.updateData(array = data)
	...
	def loadData(self):
		...
		if os.path.exists(path):
			try:
				...
				"""
				#sefl.filtersDict = getFilters(length = self.LENGTH)
				Продовжу потім..."""
				xc = active[1].value()
				yc = active[2].value()
				mc = active[3].value()
	...
	def resFilters(filters):
		"""Перерахунок для різних комбінацій фільтрів"""
		return  sp.multiply.reduce( [ self.filtersDict[i] for i in filters.split("+")] )
	...
	def uiConnect(self):
		...
		'''
		self.ui.cFilt.stateChanged[int].connect(self.cXFilt)
		self.ui.cFilt.stateChanged[int].connect(self.cYFilt)
		self.ui.sFilt.stateChanged[int].connect(self.sXFilt)
		self.ui.sFilt.stateChanged[int].connect(self.sYFilt)
		'''
